import { Dictionary, DictionaryEntry, PlaceDictionaryEntry } from "../../interface/searchInterface";


class PlaceTypeDictonary implements Dictionary {
        private entries: PlaceDictionaryEntry[];
        private termToCanonical: Map<string, string>;
        private allTerms: string[];


        constructor(styles: PlaceDictionaryEntry[]) {
            this.entries = styles;
          ...
          getTermToCanonicalMap(): Map<string, string> {
            if (this.termToCanonical.size === 0) {
              this.entries.forEach(entry => {
                [entry.feature, ...entry.synonyms, ...entry.spelling_variations, entry.region].forEach(variant => {
          ...
          getAllTerms(): string[] {
            if (!this.allTerms) {
              ...
              this.entries.forEach(entry => {
                ...
                terms.push(entry.region.toLowerCase());
...
export default PlaceTypeDictonary;